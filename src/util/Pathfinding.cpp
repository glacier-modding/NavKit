...
namespace Pathfinding {
    ...
    float4* ClosestPointOnSegment(float4* result, const float4* vV0, const float4* vV1, const float4* vP) {
        const float4 v = *vV1 - *vV0;
        const float dot = Math::dotProduct(*vP - *vV0, v);
        const float vSqrLength = Math::dotProduct(v, v);
        const float t = std::clamp(dot / vSqrLength, 0.0f, 1.0f);
        *result = *vV0 + v * t;
        return result;
    }
    ...
    char LineLineIntersect2D_G2(
        ...
        const Vec3 p3_2d(p3->X, p3->Y, 0.0f);

        const Vec3 lineA = p1_2d - p0_2d;
        const Vec3 lineB = p3_2d - p2_2d;

        const float denominator = lineB.Y * lineA.X - lineB.X * lineA.Y;

        if (denominator == 0.0f) {
            return 0;
        }

        const float uA = ((p0_2d.Y - p2_2d.Y) * lineA.X - (p0_2d.X - p2_2d.X) * lineA.Y) / denominator;
        const float uB = ((p0_2d.Y - p2_2d.Y) * lineB.X - (p0_2d.X - p2_2d.X) * lineB.Y) / denominator;
