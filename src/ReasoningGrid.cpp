...
void addWaypointsForGrid(ReasoningGrid *airg, NavPower::NavMesh *navMesh, NavKit *navKit,
        ...
        for (int yi = 0; yi < h.gridYSize; yi++) {
            ...
            for (int xi = 0; xi < h.gridXSize; xi++) {
                ...
                int edge = closestEdge(areaPointCount, areaXCoords, areaYCoords, x, y, area->m_area->m_pos.X,
                                       area->m_area->m_pos.Y, h.tolerance, navKit);
                if (edge == -1) {
...
bool waypointsAreConnected(NavKit *navKit, ReasoningGridBuilderHelper &h, ReasoningGrid *airg, Vec3 waypointPos,
...
void ReasoningGrid::build(ReasoningGrid *airg, NavPower::NavMesh *navMesh, NavKit *navKit, float spacing,
    ...
    CPPTRACE_CATCH(const std::exception& e) {
        NavKit::errorMessage = new std::string(
            "An unexpected error occurred: " + std::string(e.what()) + "\n\nStack Trace:\n" +
            cpptrace::from_current_exception().to_string());
        DialogBoxParamA(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDD_ERROR_DIALOG), Renderer::hwnd, NavKit::DialogProc,
                        0);
    } catch (...) {
