...
void Navp::setSelectedNavpAreaIndex(int index) {
    if (index == -1 && selectedNavpAreaIndex != -1) {
        Logger::log(RC_LOG_PROGRESS, ("Deselected area: " + std::to_string(selectedNavpAreaIndex + 1)).c_str());
    }
    selectedNavpAreaIndex = index;
    if (index != -1 && index < navMesh->m_areas.size()) {
        Logger::log(RC_LOG_PROGRESS, ("Selected area: " + std::to_string(index + 1)).c_str());
...
void Navp::renderNavMesh() {
    if (!loading) {
        ...
        if (showNavpIndices) {
            areaIndex = 0;
            for (const NavPower::Area &area: navMesh->m_areas) {
                navKit->renderer->drawText(std::to_string(areaIndex + 1), {
                                               area.m_area->m_pos.X, area.m_area->m_pos.Z + 0.1f, -area.m_area->m_pos.Y
                                           }, colorBlue);
                if (selectedNavpAreaIndex == areaIndex) {
                    int edgeIndex = 0;
                    for (auto vertex: area.m_edges) {
                        navKit->renderer->drawText(std::to_string(edgeIndex + 1),
...
void Navp::loadNavMesh(Navp *navp, char *fileName, bool isFromJson) {
    ...
    try {
        ...
        msg += fileName;
        msg += "'...";
        Logger::log(RC_LOG_ERROR, msg.data());
        return;
    }
...
void Navp::buildNavp(Navp *navp) {
    ...
    if (navp->navKit->sample->handleBuild()) {
        ...
        Logger::log(RC_LOG_ERROR, "Error building Navp");
    }
}
...
void Navp::drawMenu() {
    ...
    if (imguiBeginScrollArea("Navp menu", 10, navKit->renderer->height - navpMenuHeight - 10, 250, navpMenuHeight,
                             &navpScroll))
    ...
    if (imguiButton(saveNavpName.c_str(), navpLoaded)) {
        ...
        if (fileName) {
            ...
            Logger::log(RC_LOG_PROGRESS, "Done saving Navp.");
        }
    }
        ...
        if (selectedNavpAreaIndex != -1) {
            ...
            std::string newTypeString = (navMesh->m_areas[selectedNavpAreaIndex].m_area->m_usageFlags ==
                                         NavPower::AreaUsageFlags::AREA_STEPS)
                                            ? "AREA_FLAT"
                                            : "AREA_STEPS";
            Logger::log(RC_LOG_PROGRESS, ("Setting area type to: " + newTypeString).c_str());
